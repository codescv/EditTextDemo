...
- (void)updateSendCommand {
    BOOL hasRequired = [self hasEnteredText];
    if (hasRequired) {
...
- (void)send {
    ...
    for (id f in fields) {
        if ([f isKindOfClass:[TTMessageTextField class]]) {
            TTMessageTextField *m = (TTMessageTextField *) f;
            NSLog(@"title: %@ text: %@", m.title, m.text);
        } else if ([f isKindOfClass:[TTMessageRecipientField class]]) {
            TTMessageRecipientField *msg = (TTMessageRecipientField *) f;
            NSLog(@"recipients: %@", msg.recipients);
        }
    }
